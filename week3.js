...
const doubleTitles = (title) => {
  //Star Wars - The   Clone Wars
  //[Star, Wars, -, ...]
  const words = title.split(" ");
  //Regular expression = regex https://regexr.com/ (/\s+/)
  return;
};

const duplicatedWords = (title) => {
  const words = title.split(/\s+/);

  //1. We take a words array [Star, Wars, The, Clone, Wars]

  //2. Some looks for a match in the array given the instuction
  //3. Instruction is looking for oout match in the array, but starting from a point in array that is one step further than where we are

  //example: "Star" at index 0  -> is there a "Star" in array looking from index 1 -> false
  // "Wars" at index 1 -> is there a "Wars" in array looking from index 2? -> true

  //Star Wars; the clone wars (won't work here)
  return words.some((word, index) => words.includes(word, index + 1));
};

const moviesWithDuplicatedWords = movies.filter((movie) =>
  duplicatedWords(movie.title)
);

console.log(moviesWithDuplicatedWords);

const moviesWithDuplicates = movies.filter((movie) => {
  const words = movie.title.split(" "); //[Star, Wars, The, Clone, Wars] // 5
  const uniqueWords = new Set(words); //[Star, Wars, The, Clone] // 4
...
//[Star, Wars, The, Clone, Wars]
const copyWords = words.map((word) => word.toLocaleLowerCase());

const duplicatedWordMovies = movies
  .filter((movie) =>
    movie.title
      .split(" ")
      .some((word, index, copyWords) => copyWords.indexOf(word) !== index)
  )
  .map((movie) => movie.title);
...
const duplicated = movies.filter((movie) => {
  ...
  return (
    titleWords.filter((word, index) => titleWords.indexOf(word) !== index)
      .length > 0
  );
});
...
// 1.4 Create a new array that has an extra key called tag. The tag is based on the rating: Good (>= 7), Average (>= 4 and < 7), Bad (< 4)
const combinedRatings = movies.map((movie) => {
  if (movie.rating >= 7) {
    return { ...movie, quality: "Good" };
  } else if (movie.rating >= 4 && movie.rating < 7) {
    return { ...movie, quality: "Average" };
  } else {
    return { ...movie, quality: "Bad" };
  }
});

const oldArray = [1, 2, 3];
const newArray = [...oldArray, 4]; //[1,2,3,4]
const newArray1 = [oldArray, 4]; // [[1,2,3], 4]
